...
- [ ] ⚠️ **Put week 4's folder back to its finished state — §1 copies out of it.** Whatever you did to it while rehearsing, this makes tonight's carry-forward correct:
  ...
  - ⚠️ **No `week-04/Haldane` at all?** Make it first, from the same place:
...
- [ ] 💡 **And the repoint, in one line — steer them to the delete:** *"when that config is aimed at the wrong week, delete the `.vscode` folder and press F5 again. `launch.json` names the week twice and `tasks.json` three times more — editing that by hand is five chances to get it wrong"*
