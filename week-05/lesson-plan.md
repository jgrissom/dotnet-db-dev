...
| <kbd>F5</kbd> asks which debugger, or which project | Normal with several projects in one folder — **C#**, then the project. ⚠️ **The project list is genuinely hard to read and gets worse every week:** each entry is the name plus the **full path**, which runs off the end of the box, and there is a `Haldane` in every week by now. **Type `week-05` to filter it** — the path is part of the entry, so it narrows immediately. Don't narrate it; just don't fumble it. |
