...
- [ ] Then the **VS Code integrated terminal** (`` Ctrl+` ``), standing at the top — where it lives all term:
  ...
  ```
- [ ] ⚠️ **Wake the editor up — Command Palette → `.NET: Restart Language Server`** *(or `Developer: Reload Window`)*. **VS Code scanned this folder when it opened and there was nothing in it; the project you just made is one it has never seen.** Verified 2026-08-05: without this, the editor gives you no IntelliSense and no squiggles at all, while `dotnet build`/`run` work perfectly — and §5's *"look at the editor: no squiggle"* would then be true for the wrong reason and prove nothing. **Check it took:** type `Console.` somewhere in `Program.cs` and confirm a completion list appears, then delete it
