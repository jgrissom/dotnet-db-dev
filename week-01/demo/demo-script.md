...
- [ ] ⚠️ **Pre-position two terminals for §8 so you never `cd` on the projector** — one already standing in the answer key's `week-01/lab/solution`, one in a throwaway `week-01` holding a copy of `lab/starter`. **Navigating there live costs you twice:** it breaks the *never `cd`* habit you have modelled all night, in the last five minutes of doing it, and it puts the answer-key path up on the wall
...
- [ ] ⚠️ **Run it from the pre-positioned terminal — do not open the solution folder in VS Code.** The Explorer lists `Broadcast.cs`, somebody will ask you to open it, and that file is the answers to the thing they are about to spend fifty minutes on. **A terminal shows the result without showing the work**
