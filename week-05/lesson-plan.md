...
| 1:42 | 10 min | **☕ Break** |
| 1:52 | 25 min | 💥 **Two names, one object** *(slides 7–8, demo §6)*. ⭐ **No throwaway code — the whole break happens in the DEBUGGER**, on a program that is correct and stays correct. Watch `outside == muster` (false) and `outside[0] == muster[0]` (true), then mark up the copy from the Debug Console and let it run: the muster reports **nobody left to account for**, with all three still on the ice. |
