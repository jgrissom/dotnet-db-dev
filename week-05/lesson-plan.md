...
|------|----------|---------|
| 0:00 | 13 min | **Where we finished last week** *(demo §1)*. 🎯 **Open by RUNNING week 4** — press `a`, `Reyes`, Enter on an empty time, and she still says 14:45. Forty seconds, and it is a refresher rather than a re-teach. Then `week-05/Haldane` by command, **all three files carried forward** and two small edits — `AnsiConsole.Clear()` and the date. Run — the desk they know, one day on. |
| 0:13 | 25 min | 💥 **The tally that belongs to nobody** *(slides 2–3, demo §2)*. `CrewMember`, then `SignOut` holds a person instead of a name — which breaks the build everywhere a name ever went on that board, and four edits clear it. TRIPS column: 1, 1, 1 — then Lindqvist goes back out and reads **2**, which is the evidence the break needs. Then the day's total → **CS0120** → 🎯 **ask for hands** → `static` → **CS0176** → do what it says → **3, 3, 3, zero warnings**. Let it sit. |
| 0:38 | 10 min | **☕ Break** |
...
| 1:42 | 10 min | **☕ Break** |
| 1:52 | 25 min | 💥 **Two names, one object** *(slides 7–8, demo §6)*. ⭐ **No throwaway code — the whole break happens in the DEBUGGER**, on a program that is correct and stays correct. The muster crosses names off, so the copy is load-bearing: cross one off and the board does not notice. Watch `outside == muster` (false) and `outside[0] == muster[0]` (true), then mark up the copy from the Debug Console and let it run: the muster reports **nobody left to account for**, with all three still on the ice. |
...
- ⚠️ **§4 is the one segment that needs the editor, so prove it works before class.** §0 has the drill. If breakpoints are gray and never stop, **`Developer: Reload Window`** is the fix — `.NET: Restart Language Server` is the plausible-looking command that does nothing.
- ⭐ ⚠️ **The debugger's rough edges are TAUGHT this week, not apologized for — and the reason is transfer.** Two things happen that do not happen in an ordinary project, and both get a sentence: **the project list** (this folder holds a whole semester — every project copied in since week 1, `.Checks` included; theirs holds two) and **the `.vscode` folder VS Code writes itself** (`launch.json` + `tasks.json`, naming one project). **Say plainly what their own project repo does**: the same list appears, but it is two lines — their program and the checks — and `Project` is the one to pick. ⚠️ **Measured 2026-08-22, after Jeff ran it: a `.Checks` project IS in the picker** (`OutputType=Exe`, and the extension filters on `isExe && targetsDotnetCore` with no test-project exclusion). The earlier "theirs never asks" wording was wrong in eight places. ⚠️ **Without that sentence the room learns a workflow they cannot reuse** — which was the objection that put it here.
...
- **The demo commits four times, silently.** The first is the one to notice: **immediately after the branch in §1, before a line of the week is changed** — because that is the *first thing the lab asks students to do* (*"commit that before you change anything"*), and until now no demo has ever modeled it. Then the trip count fixed (§3), the muster made read-only (§6), the null handled (§7). **One push, at the end of §7. The commits themselves are silent** — git is habit from week 3 on, not content.
...
| Breakpoints are hollow/gray and never stop | The language server is asleep. Command Palette → **`Developer: Reload Window`**, then <kbd>F5</kbd>. ⚠️ `.NET: Restart Language Server` does **not** fix this and sends you down a dead end. |
...
| Somebody's checks pass but their board still shows one number | They left the `static` field and put an instance property in front of it. The check catches it behaviorally; the fix is deleting the field. |
