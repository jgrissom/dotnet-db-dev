...
| 2:20 | 25 min | **Onto GitHub** *(slides 18–19, demo §7)*. 🎯 **Opens on the one-window payoff** — the terminal has stood at the top of the repo all night, and the top is exactly where git wants to run: no move, no asymmetry, nothing to remember. Then `init` / `add` / `commit` / `push`, the private repo made live, collaborator added. **Assume zero prior git.** ⚠️ **Don't fix what `git add .` swept up.** |
