...
| 2:20 | 25 min | **Onto GitHub** *(slides 18–19, demo §7)*. ⚠️ **Opens with `cd ..`** — `dotnet` runs from `week-01`, `git` runs from the folder above it, and that asymmetry is taught on screen because it's the rule they get wrong. Then `init` / `add` / `commit` / `push`, the private repo made live, collaborator added. **Assume zero prior git.** ⚠️ **Don't fix what `git add .` swept up.** |
...
- ⭐ **Your folders match theirs exactly tonight — that's deliberate, and it's new.** You arrive with an empty `dotnet-db-coursework` (what the room builds in §2), §3 makes `week-01` inside it, §7 does `git init` at the top. **Nothing on screen is a demo shortcut they have to mentally translate**, and the `cd ..` in §7 is the same one their homework asks for. An earlier draft had the demo make a repo at `week-01`, which quietly taught the wrong place for `git init` — don't reintroduce it.
