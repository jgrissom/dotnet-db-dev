...
- [ ] ⚠️ **The console's importance is claimed by *scope and consequence*, never by listing features it doesn't have yet — and the difference is the whole discipline of this beat.** *"Everything the station knows about itself is on this screen"* and *"no second opinion"* are true on night one and stay true for sixteen weeks; they make the console matter without owing the room anything. **What follows them narrows honestly — *"tonight it answers two of those questions"* — and both are things they watch you type in the next hour** (the generator countdown is §5, `Safe to go out` is §6). ⚠️ **Do not enumerate: no roster of who's out on the ice, no incident log, no sign-out board.** Nothing tonight builds one, and week 3 is where a `List<T>` makes the log real and *"restart and it's gone"* means losing track of a person. 💡 *The one-line mention of "the log" above is furniture, not a promise — it is a thing the station has, not a thing tonight's program does.*
