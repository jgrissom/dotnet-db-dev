...
| 1:17 | 18 min | 🎯 **The table** *(slide 9, demo §4)*. **Two passes.** Padding out, a plain `Table` in — one `AnsiConsole.Write` doing what the arithmetic did *(slide 9)*. Then the colours, the banner and the border go on, `Conditions` finally gets called, and the output **is** the duty console. **The convergence beat.** |
