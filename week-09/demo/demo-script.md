...
- [ ] ⚠️ **This one runs under the debugger, so set it up before you launch.** Three breakpoints, all in `Program.cs`
  - <kbd>⌘F</kbd> for **`takenMuster = watch.SignOuts()`** — one hit. **Put the cursor just to the left of `!s.IsBack`** — directly before the `!` — then <kbd>Shift</kbd>+<kbd>F9</kbd>. ⚠️ **Clicking inside the word `IsBack` does not work.** ⚠️ **Check the BREAKPOINTS panel: the row must read column `51`** — the `!` — with the file indented four spaces per level. A column of `9` means it landed at the start of the line and it will stop once instead of once per person
