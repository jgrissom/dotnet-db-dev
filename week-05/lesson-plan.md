...
- ⚠️ **Expect a picker on <kbd>F5</kbd> and say so as it appears**, rather than being surprised: several projects in one folder means VS Code asks which one to launch. Treat it as normal, because it is.
...
- **The demo commits three times, silently**, at the same milestones the rubric asks students for: the trip count fixed (§3), the muster made read-only (§6), the null handled (§7). One push, at the end of §7. **No commentary** — git is habit from week 3 on, not content.
...
| <kbd>F5</kbd> asks which debugger, or which project | Normal with several projects in one folder — **C#**, then **week-05/Haldane**. Say it out loud as it happens so the room reads it as ordinary. |
